...
def viewExpenseInfo():
    ...
    # return a message box displaying error if no row is selecteed from the table
    ...
    # retrieving the date of the expenditure from the list
...
# function to clear the entries frmo the entry fields
def clearFields():
...
def removeExpense():
    ...
    if confirmation:
        dbconnector.exectute('DELETE FROM ExpenseTracker WHERE ID=%d'%valuesSelected[0])
        dbconnector.commit()
...
# function to delete the entries
def removeAllExpenses():
    ...
    # if the user says YES, deleting the entries from the table and executing the SQL DELETE FROM command to delete all the entires
    if confirmation:
        data_table.delete(*data_table.get_children())
        ...
        mb.showinfo('OK','The task was aborted and no expense was deleted!')
...
def addAnotherExpense():
    ...
    # if any of the fields is empty, return the message box displaying error
    if not dateField.get() or not modeOfPayment.get():
        mb.showerror('Fields empty!','Please fill all the missing fields before pressing the add button!')
    else:
        # execute the SQL INSERT INTO command
        dbconnector.execute(
...
def editExpense():
    ...
    if not data_table.selection():
        mb.showerror('No expense selected!','You have not selected any expense in the  table for us to edit; please select an expense!')
        return
    
    # calling the viewExpenseInfo() method
    viewExpenseInfo()
    
    # adding the Edit button to the selected record
...
# function to display the details of the selected expense into words
def selectedExpenseToWords():
    
    # global variables
    global data_table
    
    # returning a nessage box displaying error, if no record is selected from the table
    
    if not data_table.selection():
        mb.showerror('No expense selected!','Please select an expense from the table to read')
        return
    
    # collecting data from the selected row in dictionary format
    currentSelectedExpense = data_table.item(data_table.focus())
...
def expenseToWordsBeforeAdding():
    ...
    if not dateField.get() or not payee.get() or not description.get() or not amount.get() or not modeOfPayment.get():
        ...
        # defining the essage to be displayed in the message box
        ...
        # if the user says YES, calling the addAnother Expense() function
        if addQuestion:
            addAnotherExpense()
        else: 
            # returning a message box displaying information
            mb.showinfo('OK','Please take you time to add this record')
...
# setting the icon of the applicaiton 
main_win.iconbitmap("./piggybank.ico")

# adding necessary widgets to the window and setting the event triggers

# adding the frames to the window to provide structure to the other widgets
...
# using the grid() method to set the position of the above labels in the grid formet
...
# instantiating the DoubleVar() class to retrieve the amount detail in double data type
amount = DoubleVar()

# creating a drop-down calendar for the user to enter the date with
dateField = DateEntry(
    frameL2,
...
# creating entry fields to enter labelled data
...
modeField = OptionMenu(
    frameL2,
    modeOfPayment,
    *['Cash','Check','Credit Card','Debit Card','PayPal','ApplePay']
)

# using the config() method to configure the width, the font style, and background color of the menu
modeField.config(
    width=15,
    font=("consolas","10"),
    relief=GROOVE,
...
# adding widgets to the frameL3 frame, creating buttons to manipulate data, insert button
insertButton = Button(
    frameL3,
    text="Add Expense",
    font=("Bahnschrift Condensed","13"),
...
# adding widgets to the frameR1 frame: creating buttons to manipulate data and view button
viewButton = Button(
    frameR1,
...
# using the grid method to set the postiion of the above buttons
...
# adding widgets to the frameR2 frame - this table will display the records from the database
# creating the table to display all the entries
...
# creating a horizontal scrollbar for the table
...
# now the verticle scrollbar for the table
...
# configuring the horizontal and verticle scrollbars on the table
